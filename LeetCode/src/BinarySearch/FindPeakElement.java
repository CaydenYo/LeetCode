...
public class FindPeakElement {
     ...
     * 此时，我们不需要继续向下判断，也就不会有需要判断 nums[i]nums[i] 和上一个元素 nums[i-1]nums[i−1] 的大小的情况
     *
     * 情况2. 所有的数字以升序排列。这种情况下，我们会一直比较 nums[i]nums[i] 与 nums[i+1]nums[i+1]
     * 以判断 nums[i]nums[i] 是否是峰值元素。没有元素符合这一条件，说明处于上坡而非峰值。
     * 于是，在结尾，我们返回末尾元素作为峰值元素，得到正确结果。
     * 在这种情况下，我们同样不需要比较 nums[i]nums[i] 和上一个元素 nums[i-1]nums[i−1]，
     * 因为处于上坡是 nums[i]nums[i] 不是峰值的充分条件。
     *
     * 情况3. 峰值出现在中间某处。这种情况下，当遍历上升部分时，与情况 2 相同，
     * 没有元素满足 nums[i] > nums[i + 1]nums[i]>nums[i+1]。
     * 我们不需要比较 nums[i]nums[i] 和上一个元素 nums[i-1]nums[i−1]。
     * 当到达峰值元素时，nums[i] > nums[i + 1]nums[i]>nums[i+1] 条件满足。
     * 此时，我们同样不需要比较 nums[i]nums[i] 和上一个元素 nums[i-1]nums[i−1]。
